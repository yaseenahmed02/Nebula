...
async fn main() -> std::io::Result<()> {
    ...
    let server_socket = Arc::new(UdpSocket::bind("10.7.16.146:8082").await.unwrap());
    let mut buf = [0; 1024];
    ...
    let client_task = tokio::spawn(async move {
        loop {
            ...
            // Encrypt the received image data.
            ...
            // Send the encrypted image data back to the client in chunks.
            ...
            for chunk in encrypted_image_data.chunks(CHUNK_SIZE) {
                ...
                loop {
                    ...
                    let mut ack_buf = [0; 4];
                    match timeout(Duration::from_secs(1), server_socket.recv_from(&mut ack_buf)).await {
                        Ok(Ok((_, _))) => {
